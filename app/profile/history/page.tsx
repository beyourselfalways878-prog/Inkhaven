    ...
    return (
        <div className="container mx-auto px-6 py-10 h-[calc(100vh-80px)]">
            <div className="flex items-center gap-3 mb-6">
                <Link href="/settings" className="p-2 hover:bg-slate-950/40 backdrop-blur-2xl border border-white/10 hover:bg-slate-900 text-white/10 rounded-full transition">
                    <ChevronLeft className="w-5 h-5 text-white text-white" />
                </Link>
                <h2 className="text-2xl font-semibold text-white text-white">Saved Chats</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[80%]">
                {/* Sidebar */}
                <div className="card bg-slate-900 text-white bg-slate-900/50 border border-slate-200 border-white/10 rounded-2xl overflow-hidden flex flex-col">
                    <div className="p-4 border-b border-slate-200 border-white/5 font-semibold text-slate-700 text-white/80">
                        Recent Transcripts
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {loadingRooms ? (
                            <div className="text-center text-slate-400 text-white/40 text-sm py-10 animate-pulse">Loading...</div>
                        ) : rooms.length === 0 ? (
                            ...
                            rooms.map(room => (
                                <button
                                    ...
                                    className={`w-full text-left p-3 rounded-xl transition ${selectedRoomId === room.id ? 'bg-teal-500/20 border border-teal-500/30' : 'hover:bg-slate-950/60 backdrop-blur-3xl border border-white/20 hover:bg-slate-900 text-white/5 border border-transparent'}`}
                                >
                                    <div className="flex items-center gap-2">
                                        <MessageSquare className="w-4 h-4 text-teal-400" />
                                        <span className="text-sm text-white text-white font-medium">Stranger</span>
                                    </div>
                                    <div className="text-xs text-slate-400 text-white/40 mt-1 flex items-center gap-1">
                ...
                {/* Message View */}
                <div className="md:col-span-2 card bg-slate-900 text-white bg-slate-900/80 border border-slate-200 border-white/10 rounded-2xl overflow-hidden flex flex-col">
                    {selectedRoomId ? (
                        <>
                            <div className="p-4 border-b border-slate-200 border-white/5 flex justify-between items-center bg-slate-900 text-white bg-slate-900 absolute w-full z-10">
                                <span className="text-slate-700 text-white/80 font-medium text-sm">Room: {selectedRoomId.substring(0, 8)}...</span>
                                <button onClick={exportChat} className="btn-secondary py-1.5 px-3 text-xs flex items-center gap-1.5 hover:bg-teal-500/20 hover:text-teal-300">
                    ...
                    and flex-col-reverse makes the first item appear at the bottom! */}
                                    ...
                                    return grouped.reverse().map((g, idx) => {
                                        ...
                                        return (
                                            <div key={m.id} className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} max-w-[80%] ${isMine ? 'ml-auto' : 'mr-auto'}`}>
                                                <div className={`p-3 rounded-2xl text-sm ${isMine ? 'bg-teal-600/90 text-white rounded-tr-sm' : 'bg-slate-950/40 backdrop-blur-2xl border border-white/10 bg-slate-800 text-white text-white rounded-tl-sm border border-slate-200 border-white/5'}`}>
                                                    {m.messageType === 'text' ? m.content : <span className="italic opacity-60 text-xs">Media attachment ({m.messageType})</span>}
                                                </div>
                                                <span className="text-[10px] text-slate-400 text-white/30 mt-1">{new Date(m.createdAt).toLocaleTimeString()}</span>
                                            </div>
                                        )
                                    });
                                })()}
                                ...
                                {loadingMessages && <div className="text-center text-xs text-teal-400 py-2 animate-pulse w-full flex-shrink-0">Loading more...</div>}
                                {!hasMore && messages.length > 0 && <div className="text-center text-xs text-slate-300 text-white/20 py-2 w-full flex-shrink-0">Beginning of history</div>}
                            </div>
                        </>
                    ) : (
                        <div className="flex-1 flex flex-col items-center justify-center text-slate-400 text-white/30">
