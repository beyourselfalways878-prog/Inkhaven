          ...
          {(['all', 'pending', 'reviewed', 'resolved'] as const).map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition ${filter === status
                ...
                : 'bg-slate-950/40 backdrop-blur-2xl border border-white/10 text-slate-700 hover:bg-slate-200'
                }`}
            >
        ...
        {filteredReports.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredReports.map((report) => (
              <div
                key={report.id}
                className="rounded-lg border border-slate-200 bg-slate-900 text-white p-4 hover:bg-slate-950/60 backdrop-blur-3xl border border-white/20 transition"
              >
