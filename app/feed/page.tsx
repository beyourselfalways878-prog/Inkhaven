  ...
  return (
    <div className="container mx-auto px-4 py-10 max-w-2xl">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        {/* Sort Toggle */}
        <div className="flex items-center p-1 rounded-xl bg-slate-950/40 backdrop-blur-2xl border border-white/10 bg-slate-900 text-white/[0.04] border border-slate-200 border-white/5 w-fit shrink-0">
          <button
            onClick={() => setSort('recent')}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${
              sort === 'recent'
                ? 'bg-slate-900 text-white bg-slate-900 text-white/10 text-white text-white shadow-sm'
                : 'text-slate-500 hover:text-slate-700 text-white/40 hover:text-white/70'
            }`}
          >
            Recent
          </button>
      ...
      {/* Compose */}
      {composing ? (
        <div className="mb-6 bg-slate-900 text-white bg-slate-900 text-white/[0.04] border border-slate-200 border-white/10 rounded-2xl p-5 shadow-lg animate-in zoom-in-95 duration-200">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-slate-400 text-white/40 font-mono">
          ...
          <textarea
            ...
            className="w-full bg-transparent text-white text-white placeholder-slate-400 placeholder-white/20 text-base resize-none focus:outline-none leading-relaxed min-h-[80px]"
            rows={3}
          />
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100 border-white/5">
            <span className={`text-xs font-mono ${text.length > 250 ? 'text-red-400' : 'text-slate-400 text-white/30'}`}>
              {280 - text.length} chars left
            </span>
            <div className="flex gap-3">
              <button
                onClick={() => { setComposing(false); setText(''); }}
                className="px-4 py-1.5 rounded-xl text-sm text-slate-400 hover:text-slate-600 hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
        ...
        <button
          ...
          className="w-full mb-6 flex items-center gap-3 bg-slate-900 text-white bg-slate-900 text-white/[0.03] border border-dashed border-slate-300 border-white/10 rounded-2xl px-5 py-4 text-slate-400 text-white/30 hover:text-slate-600 hover:text-white/60 hover:border-amber-400 hover:border-amber-500/50 transition-all text-sm group"
        >
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-28 rounded-2xl bg-slate-950/40 backdrop-blur-2xl border border-white/10 bg-slate-900 text-white/[0.03] animate-pulse" />
          ))}
        </div>
      ) : posts.length === 0 ? (
        <div className="text-center py-20">
          <div className="text-5xl mb-4">🖊️</div>
          <h3 className="text-lg font-semibold text-white text-white mb-2">The wall is blank</h3>
          <p className="text-slate-500 text-white/40 text-sm">Be the first to leave a mark.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {posts.map((post) => {
            ...
            return (
              <div
                key={post.id}
                className={`group bg-slate-900 text-white bg-slate-900 text-white/[0.03] border transition-all rounded-2xl p-5 hover:shadow-md ${
                  isHot 
                    ? 'border-amber-400/50 shadow-[0_0_20px_-5px_rgba(251,191,36,0.2)] shadow-[0_0_20px_-5px_rgba(251,191,36,0.1)]' 
                    : 'border-slate-200 border-white/5 hover:border-slate-300 hover:border-white/10 hover:shadow-black/20'
                }`}
              >
                {/* Author + time */}
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-mono text-slate-600 text-white/50 font-semibold">{post.authorInkId}</span>
                      {isHot && <span className="text-[10px] text-amber-500 font-bold uppercase tracking-wider">Trending</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-slate-400 text-white/25">{timeAgo(post.createdAt)}</span>
                    <div className="flex items-center gap-1 text-xs text-slate-300 text-white/20 bg-slate-950/40 backdrop-blur-2xl border border-white/10 bg-slate-900 text-white/5 px-2 py-0.5 rounded-full">
                      <Clock size={10} />
                ...
                {/* Content */}
                <p className="text-gray-200 text-white/90 text-sm leading-relaxed whitespace-pre-wrap mb-4">
                  {post.content}
                </p>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition-all hover:scale-105 active:scale-95 ${
                      myReaction === 'resonate'
                        ...
                        : 'text-slate-400 text-white/30 hover:text-emerald-500 hover:bg-emerald-500/10'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition-all hover:scale-105 active:scale-95 ${
                      myReaction === 'dismiss'
                        ? 'bg-slate-950/60 backdrop-blur-3xl border border-white/200/15 text-slate-500 border border-slate-500/20'
                        : 'text-slate-400 text-white/30 hover:text-slate-500 hover:bg-slate-950/60 backdrop-blur-3xl border border-white/200/10'
                    }`}
                  >
                    <ThumbsDown size={15} />
                    {post.dismissCount ?? 0}
                  ...
                  {/* Net score */}
                  <span className={`ml-auto text-xs font-bold font-mono ${scorePositive ? 'text-emerald-500' : 'text-slate-400 text-white/25'}`}>
